    ...
    return Roact.createElement("ScreenGui", {
        ...
        Container = Roact.createElement("Frame",{
            ...
            Tutorial = Roact.createElement("TextLabel",{
                ...
                --Transparency = 1,
                BorderSizePixel = 0,
                Text = "Click/press on a put out a fire!"
            }),
...
function Interface:didMount()
    ...
    Modules["Generate"].GameAsset("Fire", script.Name, 5)
    game_connections.MouseScan = UserInputService.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            if scan and scan.Name == "Fire" then
                scan:Destroy()
                ...
                if self.score:getValue() >= 5 then
                    Events["sabotage_stopped"]:FireServer(script.Name)
                    Game.Stop()
                end
            end
...
function Game.Start()
    if game_start == false then
        ...
        game_handle = Roact.mount(Roact.createElement(Interface), PlayerGui, "Hunt Interface")
        game_start = true
    end
end
