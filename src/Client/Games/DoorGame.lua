    ...
    return Roact.createElement("ScreenGui", {
        ...
        Container = Roact.createElement("Frame",{
            ...
            Tutorial = Roact.createElement("TextLabel",{
                ...
                --Transparency = 1,
                BorderSizePixel = 0,
                Text = "Click/press on the barracade to lift!"
            }),
...
function Interface:didMount()
    ...
    Modules["Generate"].SinglePoint("Barracade", game_door.PrimaryPart.CFrame)
    game_connections.MouseScan = UserInputService.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            if scan and scan.Name == "Barracade" then
                scan:Destroy()
                
                self.updateScore(self.score:getValue() + 1)
                score.Text = 0 + self.score:getValue()

                if self.score:getValue() >= 1 then
                    Events["door_request"]:FireServer(game_door)
                    Game.Stop()
                end
            end
    ...
    game_connections.MobileScan = UserInputService.TouchTap:Connect(function(input)
            ...
            if scan and scan.Name == "Log" then
                scan:Destroy()
...
function Game.Start(door_object)
    if game_start == false then
        game_door = door_object
        print(door_object, " is inside the door game")
        game_handle = Roact.mount(Roact.createElement(Interface), PlayerGui, "Door Game")
        game_start = true
    end
end
